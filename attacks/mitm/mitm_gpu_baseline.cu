/*
 * mitm_gpu_baseline.cu — KeeLoq baseline Slide+MitM GPU profile (tp=16, tc=16, to=16)
 *
 * This file is the fixed-profile CUDA baseline for Phase 4.
 * It keeps the proven 16/16/16 logic and adds validation-friendly runtime flags:
 *   - --pairs-log2 N       (8..16)
 *   - --inject-slid-pair   (deterministic fast correctness checks)
 *   - --max-k0 N           (bounded scans)
 *
 * Reference: Indesteege et al., EUROCRYPT 2008.
 */
...
#define PROFILE_TP 16
#define PROFILE_TC 16
#define PROFILE_TO 16
...
uint32_t keeloq_dec_one_round(uint64_t key, uint32_t sp, int ki)
{
...
__device__ __forceinline__
uint32_t make_pstar(uint32_t pj, uint32_t underP)
{
    return ((pj & 0xFFFFu) << 16) | (underP & 0xFFFFu);
}

__device__ __forceinline__
uint32_t make_xstar(uint32_t xi, uint32_t upj)
{
    ...
    uint32_t N)
{
    ...
    X[i] = enc_rounds(kk0, d_P[i], 0, PROFILE_TP);
    Y[i] = dec_rounds(kk0, d_C[i], 0, PROFILE_TP);
}
    ...
    uint32_t upj_batch_size)
{
    ...
    uint32_t upj_off = blockIdx.z;
    if (upj_off >= upj_batch_size) return;
    uint32_t upj = upj_start + upj_off;
    if (upj >= K0_SPACE) return;

    const uint32_t *Y = d_Y + (size_t)k0_idx * N;

    uint32_t pj = d_P[j];
    uint32_t pstar = make_pstar(pj, upj);
    uint64_t k3 = extract_key_bits(pstar, pj, 48, PROFILE_TC);
    uint32_t ystar = dec_rounds(k3, Y[j], 48, PROFILE_TC);
#if ENABLE_GPU_STATS
    atomicAdd(&d_right_candidates, 1ULL);
#endif

    uint32_t batch_idx = k0_idx * upj_batch_size + upj_off;
    uint32_t rec_idx = batch_idx * N + j;
    uint32_t bucket = ystar & 0xFFFFu;
    DevHTEntry *e = &d_ht[rec_idx];
    e->ystar = ystar;
    e->pj = pj;
    e->k3_bits = (uint32_t)(k3 >> 48);
    ...
    uint32_t upj_batch_size)
{
    ...
    uint32_t xi = X[i];
    uint32_t xstar = make_xstar(xi, upj);
    ...
    for (int32_t rec_idx = d_ht_head[(size_t)batch_idx * HT_BUCKETS + probe];
        ...
        uint32_t pstar_full = make_pstar(e->pj, upj);
        uint64_t k3 = (uint64_t)e->k3_bits << 48;
        uint64_t k2_c;
        if (!mid_bits_agree(cstar, e->ystar, xstar, pstar_full, 32, 16, &k2_c)) continue;

        uint64_t cand = kk0 | k1 | k2_c | k3;
    #if ENABLE_GPU_STATS
...
int main(int argc, char **argv)
{
    ...
    printf("KeeLoq baseline Slide+MitM Attack — GPU profile 16/16/16\n");
    ...
    size_t ht_records = (size_t)alloc_k0_batch * UPJ_BATCH * pair_count;
    size_t ht_bytes = ht_records * sizeof(DevHTEntry);
    size_t next_bytes = ht_records * sizeof(int32_t);
    size_t head_bytes = (size_t)alloc_k0_batch * UPJ_BATCH * HT_BUCKETS * sizeof(int32_t);
    size_t total_dev_bytes = pair_bytes * 2 + XY_bytes * 2 + ht_bytes + next_bytes + head_bytes;

    if (ht_records > (size_t)INT32_MAX + 1u) {
        fprintf(stderr, "Batch geometry exceeds the exact int32 hash-chain index limit. Reduce K0_BATCH or UPJ_BATCH.\n");
        free(h_P); free(h_C);
        return 2;
    }
    size_t memory_reserve = prop.totalGlobalMem / 20u;
    if (total_dev_bytes > prop.totalGlobalMem - memory_reserve) {
        fprintf(stderr, "GPU buffers need %.2f GiB; reduce K0_BATCH or UPJ_BATCH.\n",
    ...
    for (uint32_t k0_start = 0; k0_start < max_k0 && !h_found; k0_start += K0_BATCH) {
        ...
        for (uint32_t upj_start = 0; upj_start < K0_SPACE && !h_found; upj_start += UPJ_BATCH) {
            uint32_t upj_batch_sz = UPJ_BATCH;
            if (upj_start + upj_batch_sz > K0_SPACE) upj_batch_sz = K0_SPACE - upj_start;

            dim3 grid3d(batch_sz, n_pair_blocks, upj_batch_sz);
            size_t active_head_bytes = (size_t)batch_sz * upj_batch_sz * HT_BUCKETS * sizeof(int32_t);
            CUDA_CHECK(cudaMemsetAsync(d_ht_head, 0xFF, active_head_bytes));
            build_ht_kernel<<<grid3d, block>>>(d_P, d_Y, d_ht, d_ht_next, d_ht_head, batch_sz, pair_count, upj_start, upj_batch_sz);
            probe_ht_kernel<<<grid3d, block>>>(d_P, d_C, d_X, d_ht, d_ht_next, d_ht_head, k0_start, batch_sz, pair_count, upj_start, upj_batch_sz);

            if (((upj_start + upj_batch_sz) & 0xFFFu) < UPJ_BATCH) {
